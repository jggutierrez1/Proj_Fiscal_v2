...
function InternetAvailable: Boolean; // Only DF� / RAS
...
function IcmpSendEcho(const IcmpHandle: DWORD; const DestinationAddress: IPAddr; const RequestData: Pointer;
  const RequestSize: WORD; const RequestOptions: PIP_OPTION_INFORMATION; const ReplyBuffer: Pointer;
  const ReplySize: DWORD; const TimeOut: DWORD): DWORD; stdcall; external 'icmp.dll';
function GetNetworkName(IPAddr: string): string;
...
begin
  ...
  try
    ...
    try
      if IcmpSendEcho(hICMP, MAKELONG(MAKEWORD(StrToInt(Class1), StrToInt(Class2)), MAKEWORD(StrToInt(Class3),
        StrToInt(Class4))), nil, 0, nil, pierWork, dwSize, TimeOut) = 0 then
        Result := False
      else
        Result := True;
    finally
...
function CreateNetResourceList(ResourceType: DWORD; NetResource: PNetResource; out Entries: DWORD;
